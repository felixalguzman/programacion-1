...
public class RegistrarVenta extends JDialog {
	 ...
	 * 
	 */
	public RegistrarVenta(Vendedor pvend, String title,  boolean option,  Clientes client, Venta pventa, Vehiculo vehiculo, int verSoloDetalles) {
		setTitle(title);
		...
		btnSalvar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(!nombreCliente.getText().equalsIgnoreCase("") && clienteCedula.getText().length()==13  && !clienteTelefono.getText().equalsIgnoreCase("") && !direccionCliente.getText().equalsIgnoreCase("") && !apellidoCliente.getText().equalsIgnoreCase(""))
				{
					...
					venta.setCodigo(vend.getCantVenta()+1);
					
					System.out.println(venta.getCodigo());
					
					vend.registrarVenta(venta);
					//vend.registrarCliente(client);
										
					vend.eliminarVehiculoSinIndice(vehi);
					VehiculosVenta.cargarVehiculo();
					JOptionPane.showMessageDialog(null, "Venta registrada satisfactoriamente", null, JOptionPane.INFORMATION_MESSAGE, null);
					dispose();
		...
		lblColor.setBounds(160, 32, 35, 14);
		...
		lblPrecio.setBounds(301, 32, 40, 14);
		panel_2.add(lblPrecio);
		
		precioVehiculoCliente = new JTextField();
		precioVehiculoCliente.setBounds(346, 29, 100, 20);
